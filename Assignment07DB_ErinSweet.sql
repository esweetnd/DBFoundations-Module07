...
-- 2017-01-01,ErinSweet,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_ErinSweet')
	 Begin 
	  Alter Database [Assignment07DB_ErinSweet] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_ErinSweet;
	 End
	Create Database Assignment07DB_ErinSweet;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_ErinSweet;
...
SELECT ProductName,  datename(m,InventoryDate)+' '+cast(datepart(yyyy,InventoryDate) as varchar) AS InventoryDate, Count
FROM vProducts
JOIN vInventories
...
AS
SELECT TOP 10000 ProductName,  datename(m,InventoryDate)+' '+cast(datepart(yyyy,InventoryDate) as varchar) AS InventoryDate, Count
FROM vProducts
JOIN vInventories
ON vInventories.ProductID = vProducts.ProductID
ORDER BY ProductName, InventoryDate, Count;
...
AS
SELECT TOP 10000 CategoryName,  datename(m,InventoryDate)+' '+cast(datepart(yyyy,InventoryDate) as varchar) AS InventoryDate, Count
FROM vCategories
JOIN vProducts
ON vCategories.CategoryID = vProducts.CategoryID
JOIN vInventories
...
AS
SELECT TOP 10000 ProductName, InventoryDate, (Count), LAG(SUM(Count)) OVER(ORDER BY MONTH(InventoryDate)) AS PreviousCount
...
SELECT ISNULL(ProductName, 0) AS ProductName, ISNULL(InventoryDate,0) AS InventoryDate,ISNULL(Count,0) AS Count, ISNULL(PreviousCount, 0) AS PreviousCount
...
SELECT TOP 10000 ProductName, InventoryDate, (Count), LAG(SUM(Count)) OVER(ORDER BY MONTH(InventoryDate)) AS PreviousCount,
[QtyChangeKPI] = Case 
   When (Count) > PreviousCount Then 1
   When (Count) = PreviousCount Then 0
   When (Count) < PreviousCount Then -1
   End
FROM vProductInventoriesWithPreviouMonthCounts
GROUP BY ProductName, InventoryDate,(Count), PreviousCount;
GO
...
RETURNS TABLE
AS
RETURN SELECT ProductName, InventoryDate, (Count), LAG(SUM(Count)) OVER(ORDER BY MONTH(InventoryDate)) AS PreviousCount,
[QtyChangeKPI] = Case 
	When (Count) > PreviousCount Then 1
	When (Count) = PreviousCount Then 0
	When (Count) < PreviousCount Then -1)
